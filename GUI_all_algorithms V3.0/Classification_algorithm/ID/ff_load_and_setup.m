...
params.VD = 650; % [mm] distance from screen
...
[header1Data, header2, columnData] = readFromLog(fileID);
fclose(fileID);
...
test.DPI = DPI;
if isempty(header1Data{19})
    test.FRAMERATE = 120;
else
    test.FRAMERATE = double(header1Data{19});
end
...
if (isempty(columnData{1,1}))
    ...
    if (params.fDiscardOutliers)
        ...
        params = discardLostGaze(columnData{1,cols.stim_idx}, params);
        ...
        sampleValidityAvatarRows = discardLastAvatarRows(columnData{1,cols.stim_idx});
...
% after it ends.
[avatarRows,~] =  find(columnData{cols.stim_idx} < 0);
index = 1;
if size(avatarRows,1)>0
    index = size(avatarRows,1);
end
for i=1:size(avatarRows,1)-1
    if (avatarRows(i)+1 ~= avatarRows(i+1) && columnData{cols.stim_idx}(avatarRows(i)+1)~= -1000 ...
            && columnData{cols.stim_idx}(avatarRows(i)+1)~= 555)
...
V = computeLinVelocity(X,Y,test.FRAMERATE,test.DPI,rw2zero); % V is returned in [mm/s]
test.V = V; 
Vang = computeAngVelocity(X,Y,test.FRAMERATE,params.VD,test.DPI,rw2zero); % Vang is returned in [deg/s]
test.Vang = Vang;
...
Diam = columnData{1,cols.stim_size};
SSize = Diam;
...
X = columnData{1,cols.stim_X} / (scale_factor); % Offset correction
Y = columnData{1,cols.stim_Y} / (scale_factor); % Offset correction
...
SId = double(columnData{1,cols.stim_idx});
filtDer = [1; -1]; % x - x_1
...
function [columnHeader1Data, columnHeader2, columnData] = readFromLog(fileID)
...
while index < size(rr2delLostGaze,1)
    if(stim_idx(rr2delLostGaze(index)-1,1) ~= 555) %Marca como no validos files de ronda previa
        id = stim_idx(rr2delLostGaze(index)-1,1);
        indexRoundIgnore = rr2delLostGaze(index)-1;
        
        while stim_idx(indexRoundIgnore,1) == id
        ...
        while stim_idx(rr2delLostGaze(index)-1,1) == 555 && index < size(rr2delLostGaze,1)
            index = index + 1;
        end
...
validSamples = ones(size(stim_idx,1),1);
validSamples(rr2del) = 0;
params.sampleValidity = validSamples;
return;

function validSamples = discardLastAvatarRows(stim_idx)
% Detects the start and end index of the two avatar appears and marks as
% outliers the last one.
filtDer = [1; -1]; % x - x_1
avatarLostSamples = zeros(size(stim_idx,1),1);
avatarLostSamples(((stim_idx == -1000) + (stim_idx == 555)) > 0) =  1;
...
function V = computeLinVelocity(X,Y,framerate,dpi,rw2zero)
...
function Vang = computeAngVelocity(X,Y,framerate,VD,dpi,rw2zero)

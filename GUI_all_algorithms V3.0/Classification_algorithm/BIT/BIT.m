function [output_id,output_totaal]=BIT(stamps,param)

% Version December 3, 2011
% By: Ralf van der Lans, Hong Kong University of Science and Technology
% Contact: rlans@ust.hk
%
% Please cite: van der Lans, R., M. Wedel, and R. Pieters (2011) "Defining Eye-Fixation Sequences Across Individuals and Tasks: 
%              The Binocular-Individual Threshold (BIT) Algorithm," Behavior Research Methods, 43, 239-257.
%
%--------------------------------------------------------------------------
%
% INPUT FOR TWO EYES:
% stamps       : (n_samples x 7)-matrix with data stamps measured by the
%                eye-tracker
%                first column : number of the individual
%                second column: time stamp (ms)
%                third column : x-coordinate of POR left eye
%                fourth column: x-coordinate of POR right eye
%                fifth column: y-coordinate of POR left eye
%                sixth column: y-coordinate of POR right eye
%                seventh column : distance eye-screen
% OPTIONAL INPUT PARAMETERS
% param.min_samples  : minimum number of samples a fixation should consist of (default = 3)
% param.n_lost       : maximum number of consequitive samples not tracked within a fixations (default = 3)
% param.perc_control : control percentage in quality control default : 1-sqrt(.001)
% param.filename     : filename with fixations (txt and .mat) default : BIT_output
% param.max_x        : screen width (pixels): default = 1600
% param.max_y        : screen height (pixels) default = 1200
%-------------------------------------------------------------------------
% INPUT FOR ONE EYE:
% stamps       : (n_samples x 5)-matrix with data stamps measured by the
%                eye-tracker
%                first column : number of the individual
%                second column: time stamp (ms)
%                third column : x-coordinate of POR
%                fourth column: y-coordinate of POR
%                fifth column : distance eye-screen
% min_samples  : minimum number of samples a fixation should consist of
% n_lost       : maximum number of consequitive samples not tracked within a fixations
% perc_control : control percentage in quality control (usually 1-sqrt(.001))
%
%---------------------------------------------------------------------
% OUTPUT TWO EYES:
% output_id    : struct (for each individual)
%                id: person_id
%                mu: robust mean
%                sigma: robust covariance matrix
%                perc_data: percentage of data correctly measured
%                fix: struct
%                     fixations: (number of fixations x 5 matrix)
%                                first column: person_id
%                                second column: fixation duration (ms)
%                                third column: x-coordinate (average both eyes)
%                                fourth column: y-coordinate (average both eyes)
%                                fifth column: number of stamps
%                     stamps:    (number of stamps x 8 matrix)
%                                first column: person_id
%                                second column: timestamp (ms)
%                                third column: measured x-coordinate (left)
%                                fourth column: measured x-coordinate (right)
%                                fifth column: measured y-coordinate (left)
%                                sixth column: measured y-coordinate (right)
%                                seventh column: distance
%                                eight column: to which fixation number stamp belongs
%                     stamps_positions:     (number of stamps x 6 matrix)
%                                           first column: person_id
%                                           second column: timestamp (ms)
%                                           third column:  x-coordinate fixation (0 belongs to saccade)
%                                           fourth column: y-coordinate fixation (0 belongs to saccade)
%                                           fifth column: measured distance
%                                           sixth column: to which fixation stamp belongs (0 = saccade)
% output_totaal   : struct (one list for all respondents)
%                   id: list of person-ids
%                   mu: list of robust means
%                   sigma: list of robust [std_x std_y cov(x,y)]
%                   fixations: list of all fixations (see fixations output_id)
%                   stamps_fixations: see output_id.stamps
%                   stamps_positions: see output_id
%------------------------------------------------------------------------
% OUTPUT ONE EYE:
% output_id    : struct (for each individual)
%                id: person_id
...
%                                fifth column: number of stamps
%                     stamps:    (number of stamps x 6 matrix)
%                                first column: person_id
%                                second column: timestamp (ms)
%                                third column: measured x-coordinate
%                                fourth column: measured y-coordinate
%                                fifth column: measured distance
%                                sixth column: to which fixation stamp belongs (0 = saccade)
%                     stamps_positions:     (number of stamps x 6 matrix)
%                                           first column: person_id
...
%                   sigma: list of robust [std_x std_y cov(x,y)]
%                   fixations: list of all fixations (see fixations output_id)
%                   stamps_fixations: see output_id
%                   stamps_positions: see output_id
%

% check parameter settings
if nargin == 1   
    param.min_samples = 5; %%%%%%%%%%%% original: 3              
    param.n_lost = 3;                   
    param.perc_control = 1-sqrt(.001);  
    param.filename = 'BIT_ouput';       
    param.max_x = 2160;                 
    param.max_y = 1440;                 
else
    names = fieldnames(param);
    if isempty(strmatch('min_samples',names,'exact'))
        param.min_samples = 3;
    end
    if isempty(strmatch('n_lost',names,'exact'))
        param.n_lost = 3;
    end
    if isempty(strmatch('perc_control',names,'exact'))
        param.perc_control = 1-sqrt(.001);
    end
    if isempty(strmatch('filename',names,'exact'))
        param.filename = 'BIT_output';
    end
    if isempty(strmatch('max_x',names,'exact'))
        param.max_x = 1600;
    end
    if isempty(strmatch('max_y',names,'exact'))
        param.max_y = 1200;
    end
end
min_samples = param.min_samples;
n_lost = param.n_lost;
perc_control = param.perc_control;
filename = param.filename;
max_x = param.max_x;
max_y = param.max_y;

options.lts=1;    % input for fastmcd algorithm to get no plots as output.

time_of_stamp = stamps(2,2)-stamps(1,2);

one_eye = size(stamps,2)==5;


% array of measured individuals
index_id = unique(stamps(:,1));
fixations = [];
stamps_fixations = [];
stamps_positions = [];
if one_eye  % single eye
    mu_totaal = zeros(length(index_id),2);
    sigma_totaal = zeros(length(index_id),3);
    
    %initialize
    stamps_fixations = zeros(length(stamps),6);
    stamps_positions = zeros(length(stamps),6);
    
else % both eyes
    mu_totaal = zeros(length(index_id),4);
    sigma_totaal = zeros(length(index_id),10);
    
    %initialize
    stamps_fixations = zeros(length(stamps),8);
    stamps_positions = zeros(length(stamps),6);
end

% create output structures
fix = struct('fixations',[],'stamps',[],'stamps_positions',[]);
if size(stamps,2)==5
    output_id = struct('id',0,'mu',zeros(1,2),'sigma',zeros(2,2),'perc_data',0);
else
    output_id = struct('id',0,'mu',zeros(1,4),'sigma',zeros(4,4),'perc_data',0);
end

counter = 0;
for i=index_id'
    counter = counter+1;
    output_id(counter).id = i;
    
    index_stamps = find(stamps(:,1)==i);
    n_stamps=length(index_stamps);
    stamps_id = stamps(index_stamps,:);
    
    % determine correctly measured stamps: x and y coordinates > 0 and smaller
    % than specified (max_x,max_y)
    if one_eye
        logical_correct = logical(position_check(stamps_id(:,3:4),max_x,max_y));
    else
        logical_correct = logical(position_check(stamps_id(:,3:6),max_x,max_y));
    end
    ind_correct = find(sum(logical_correct,2));
    stamps_correct = stamps_id(ind_correct,:);
    
    lost = ind_correct(2:end) - ind_correct(1:end-1)-1;
    lost = [0 lost']';
           
    dist_data = [];
    dist_data(:,1:2) = stamps_correct(2:end,1:2);
    if one_eye
        dist = stamps_correct(2:end,3:4) - stamps_correct(1:end-1,3:4);
        dist_data(:,3:4) = dist;
    else
        dist = stamps_correct(2:end,3:6) - stamps_correct(1:end-1,3:6);
        dist(~logical_correct(ind_correct(2:end),3),[1 3])=NaN;
        dist(~logical_correct(ind_correct(2:end),4),[2 4])=NaN;
        dist(~logical_correct(ind_correct(1:end-1),4),[2 4])=NaN;
        dist(~logical_correct(ind_correct(1:end-1),3),[1 3])=NaN;
        dist_data(:,3:6) = dist;
        % only include valid distances
        valid_dist = sum(isnan(dist),2)~=4;
        dist=dist(valid_dist,:);
        dist_data=dist_data(valid_dist,:);
        if sum(valid_dist>0)
            while ~valid_dist(1)
                % delete first stamps_correct
                stamps_correct = stamps_correct(2:end,:);
                ind_correct = ind_correct(2:end);
                valid_dist = valid_dist(2:end);
            end
            stamps_correct=stamps_correct([1 find(valid_dist)'+1]',:);
            ind_correct=ind_correct([1 find(valid_dist)'+1]',:);
        else
            stamps_correct=[];
            ind_correct=[];
        end
    ...
    output_id(counter).perc_data = sum(sum(isnan(dist),2)==0)/n_stamps;
    if sum(sum(isnan(dist),2)==0)>8    % At least 8 observations
        % determine Robust estimates
        res = fastmcd(dist,options);
        mu = res.center;
        Sigma = res.cov;
               
        % determine potential outliers (outlier = indicated by 1)
        outliers = 1-fixation_check_normal(dist,mu,Sigma,perc_control);
        outliers = [0 outliers']';
        
        % check whether outliers belong to potential fixations (1-step ahead
        % forecasting)
        [outliers_one_step,outliers]=one_step_ahead(outliers,stamps_correct,mu,Sigma,perc_control,lost,n_lost);
        
        % determine fixations
        [fixations_id,fixations_stamps_id]=determine_fix(outliers,outliers_one_step,min_samples,ind_correct,n_stamps,n_lost,stamps_id,lost,max_x,max_y,time_of_stamp);
        
        % add fixations to list
        
        fixations = [fixations' fixations_id']';
        stamps_fixations_current = [stamps(index_stamps,:) fixations_stamps_id];
        stamps_fixations(index_stamps,:) = stamps_fixations_current; %[stamps_fixations stamps_fixations_current'];  % CHANGE TRANSPOSE
        if one_eye
            stamps_positions_current = stamps_fixations_current;
        else
            stamps_positions_current = stamps_fixations_current(:,[1:4 7 8]);
        end
        for j=1:size(fixations_id,1)
            n=sum(stamps_positions_current(:,6)==j);
            stamps_positions_current(stamps_positions_current(:,6)==j,3:4)=repmat(fixations_id(j,3:4),n,1);
        end
        stamps_positions_current(stamps_positions_current(:,6)==0,3:4)=0;
        
        %%% change transpose
        stamps_positions(index_stamps,:) = stamps_positions_current;%[stamps_positions stamps_positions_current'];
        
        output_id(counter).mu = mu;
        output_id(counter).sigma = Sigma;
        output_id(counter).fix.fixations = fixations_id;
        output_id(counter).fix.stamps = stamps_fixations_current;
        output_id(counter).fix.stamps_positions = stamps_positions_current;
        
        mu_totaal(counter,:) = mu;
        if one_eye
            sigma_totaal(counter,:) = [sqrt(Sigma(1,1)) sqrt(Sigma(2,2)) Sigma(1,2)];
        else
            sigma_totaal(counter,:) = [sqrt(diag(Sigma))' Sigma(1,2:4) Sigma(2,3:4) Sigma(3,4)];
        end
        i
    end
end
output_totaal.id=index_id;
output_totaal.mu=mu_totaal;
output_totaal.sigma=sigma_totaal;
output_totaal.fixations=fixations;
output_totaal.stamps_fixations=stamps_fixations;
output_totaal.stamps_positions=stamps_positions;

% write to text files
dlmwrite([filename '_fix.txt'], fixations,'precision', '%.0f','delimiter', '\t', 'newline', 'pc');
dlmwrite([filename '_fix_stamps.txt'], stamps_fixations, 'precision', '%.0f','delimiter', '\t', 'newline', 'pc');
dlmwrite([filename '_fix_positions.txt'], stamps_positions, 'precision', '%.0f','delimiter', '\t', 'newline', 'pc');

% write MATLAB output
save(filename,'output_id','output_totaal');

%--------------------------------------------------------------------------
function fixations=delete_fix_small(fixations_potential,min_samples,outliers)

n_fix = max(fixations_potential);

positive_values = fixations_potential>0;

% delete starting outliers
[a,b,c]=unique(fixations_potential,'first');
outliers(b)=0;
positive_values = logical(max(positive_values - outliers',0));

[a,b1,c]=unique(fixations_potential(positive_values),'first');
[a,b2,c]=unique(fixations_potential(positive_values),'last');

n_samples = ones(1,n_fix);
n_samples(a) = b2-b1+1;

delete_fix = find(n_samples<min_samples);

delete = ismember(fixations_potential,delete_fix);
fixations_potential(delete)=0;

fix_nr=setdiff(1:n_fix,delete_fix);

[temp,fixations_potential]=ismember(fixations_potential,[0 fix_nr]);
fixations=fixations_potential-1;

%--------------------------------------------------------------------------
function [fixations,fixations_stamps]=determine_fix(outliers,outliers_one_step,min_samples,ind_correct,n_stamps,n_lost,stamps,lost,max_x,max_y,time_of_stamp)

one_eye=size(stamps,2)==5;

% determine potential fixations
fixations_potential = [1+cumsum(outliers_one_step)'];

% filter out fixations of less than min_samples samples
fixations_potential = delete_fix_small(fixations_potential,min_samples,outliers);

% index potential fixation samples (positive falues in the
% fixations_potential array)
positive_values = fixations_potential>0;
ind_positive_values = find(positive_values);

% identify potential fixations that are interrupted by more than n_lost
% samples
not_connected = find(lost>n_lost);

% split these interrupted potential_fixations
for i=not_connected'
    fixations_potential(i:end)=(fixations_potential(i:end)+1).*positive_values(i:end);
end
% filter out fixations of less than min_samples samples
fixations_potential = delete_fix_small(fixations_potential,min_samples,outliers);
positive_values = fixations_potential>0;
...
% generate array for fixations_stamps (same size as n_stamps)
fixations_stamps=zeros(n_stamps,1);
fixations_stamps(ind_correct)=fixations_potential;

% connect interruptions, by find first and last samples of each fixation
[a,b1,c]=unique(fixations_potential(positive_values),'first');
b1 = ind_positive_values(b1);
b1 = ind_correct(b1);
[a,b2,c]=unique(fixations_potential(positive_values),'last');
b2 = ind_positive_values(b2);
b2 = ind_correct(b2);

for i=1:length(b1)
    fixations_stamps(b1(i):b2(i))=i;
end

% create fixations matrix
n_fix = max(fixations_stamps);
fixations = zeros(n_fix,5);

% individual number
fixations(:,1)=stamps(1,1);

% determine fixation duration
fix_values = fixations_stamps>0;
ind_fix_values = find(fix_values);
[a,begin_fix,c]=unique(fixations_stamps(fix_values),'first');
begin_fix = ind_fix_values(begin_fix);

[a,end_fix,c]=unique(fixations_stamps(fix_values),'last');
end_fix = ind_fix_values(end_fix);

fixations(:,2)=stamps(end_fix,2)-stamps(begin_fix,2) + time_of_stamp;
fixations(:,5)=end_fix-begin_fix+1;

% x and y coordinates
for i=1:n_fix
    id_fix_stamps = ind_correct(fixations_stamps(ind_correct)==i);
    if one_eye
        fixations(i,3) = mean(stamps(id_fix_stamps,3));
        fixations(i,4) = mean(stamps(id_fix_stamps,4));
    else
        idx_left = id_fix_stamps(find(stamps(id_fix_stamps,3)>0 & stamps(id_fix_stamps,3)<max_x));
        idx_right = id_fix_stamps(find(stamps(id_fix_stamps,4)>0 & stamps(id_fix_stamps,4)<max_x));
        idy_left = id_fix_stamps(find(stamps(id_fix_stamps,5)>0 & stamps(id_fix_stamps,5)<max_x));
        idy_right = id_fix_stamps(find(stamps(id_fix_stamps,6)>0 & stamps(id_fix_stamps,6)<max_x));
        
        ndx_left = length(idx_left);
        ndx_right = length(idx_right);
        ndy_left = length(idy_left);
        ndy_right = length(idy_right);
        if ndx_left && ndx_right>0
            fixations(i,3) = (ndx_left*mean(stamps(idx_left,3)) + ndx_right*mean(stamps(idx_right,4)))/(ndx_left+ndx_right);
        elseif ndx_left==0
             fixations(i,3) = (ndx_right*mean(stamps(idx_right,4)))/(ndx_right);
        else
             fixations(i,3) = (ndx_left*mean(stamps(idx_left,3)))/(ndx_left);
        end
        if ndy_left>0 && ndy_right>0
            fixations(i,4) = (ndy_left*mean(stamps(idy_left,5)) + ndy_right*mean(stamps(idy_right,6)))/(ndy_left+ndy_right);
        elseif ndy_left==0
            fixations(i,4) = (ndy_right*mean(stamps(idy_right,6)))/(ndy_right);
        else 
            fixations(i,4) = (ndy_left*mean(stamps(idy_left,5)))/(ndy_left);
        end
    end
end
...
% options.lts=1 is the algoritm without a plot and other screen output
      ...
      if ~fine
		...
		for i=1:nsamp
         ...
         elseif fine 
            ...
            covmat=covmat+sqrt(eps)*eye(length(covmat));  %added by Ralf
...
%----------------------------------------------------------------------------------------
function validity=fixation_check_normal(dist,mu,sigma,perc_control)

% checks whether distances are likely to come from normal distribution
% N(mu,sigma). 
%
% INPUT:
% dist : (nx2)-matrix with distances (x and y coordinates respectively) or
% (nx4) when both eyes
% mu   : (2x1)-vector with mean x and mean y or (4x1)
% sigma: (2x2) covariance matrix or (4x4)
% perc-control: probability that determines whether distance is likely to
% come from normal distribution (p>perc_control is assigned likely)
% 
% OUTPUT:
% validity: 1 if Prob(dist|mu,sigma)>perc_control, zero otherwise

one_eye=size(dist,2)==2;
n_dist = size(dist,1);

if ~one_eye
    left_ok = ~isnan(dist(:,1));
    right_ok = ~isnan(dist(:,2));
    both_ok = logical(left_ok.*right_ok);
end

diff_mu = dist - repmat(mu,n_dist,1);
inv_sigma=inv(sigma);

Chi_sq_0 = zeros(n_dist,1);
if one_eye
    Chi_sq_0 = Chi_sq_0 + (diff_mu(:,1).^2).*inv_sigma(1,1);
    Chi_sq_0 = Chi_sq_0 + 2.*diff_mu(:,1).*diff_mu(:,2).*inv_sigma(1,2);
    Chi_sq_0 = Chi_sq_0 + (diff_mu(:,2).^2).*inv_sigma(2,2);
else
    Chi_sq_0(left_ok) = Chi_sq_0(left_ok) + (diff_mu(left_ok,1).^2).*inv_sigma(1,1);
    Chi_sq_0(both_ok) = Chi_sq_0(both_ok) + 2.*diff_mu(both_ok,1).*diff_mu(both_ok,2).*inv_sigma(1,2);
    Chi_sq_0(right_ok) = Chi_sq_0(right_ok) + (diff_mu(right_ok,2).^2).*inv_sigma(2,2);
    Chi_sq_0(left_ok) = Chi_sq_0(left_ok) + (diff_mu(left_ok,3).^2).*inv_sigma(3,3);
    Chi_sq_0(right_ok) = Chi_sq_0(right_ok) + (diff_mu(right_ok,4).^2).*inv_sigma(4,4);
    Chi_sq_0(left_ok) = Chi_sq_0(left_ok) + 2.*diff_mu(left_ok,1).*diff_mu(left_ok,3).*inv_sigma(1,3);
    Chi_sq_0(both_ok) = Chi_sq_0(both_ok) + 2.*diff_mu(both_ok,1).*diff_mu(both_ok,4).*inv_sigma(1,4);
    Chi_sq_0(both_ok) = Chi_sq_0(both_ok) + 2.*diff_mu(both_ok,2).*diff_mu(both_ok,3).*inv_sigma(2,3);
    Chi_sq_0(right_ok) = Chi_sq_0(right_ok) + 2.*diff_mu(right_ok,2).*diff_mu(right_ok,4).*inv_sigma(2,4);
    Chi_sq_0(both_ok) = Chi_sq_0(both_ok) + 2.*diff_mu(both_ok,3).*diff_mu(both_ok,4).*inv_sigma(3,4);
end
validity=zeros(length(Chi_sq_0),1);
if one_eye
    validity = chi2cdf(Chi_sq_0,2) < perc_control;
else
    validity(left_ok) = chi2cdf(Chi_sq_0(left_ok),2) < perc_control;
    validity(right_ok) = chi2cdf(Chi_sq_0(right_ok),2) < perc_control;
    validity(both_ok) = chi2cdf(Chi_sq_0(both_ok),4) < perc_control;
end

%-------------------------------------------------------------------------
function [outliers_new,outliers]=one_step_ahead(outliers,stamps,mu,Sigma,perc_control,lost,n_lost)

one_eye=size(stamps,2)==5;
outliers_new = outliers;

% one-step ahead distances
if one_eye
    dist2 = stamps(3:end,3:4) - stamps(1:end-2,3:4);
else
    dist2 = stamps(3:end,3:6) - stamps(1:end-2,3:6);
end
% situation [x 1 1]: check the second 1 with x
candidate_checks = (outliers_new(3:end) == 1) & (outliers_new(2:end-1)==1);
dist_check = dist2(candidate_checks,:);

outliers_update = 1-fixation_check_normal(dist_check,mu,Sigma,perc_control);

outlier_checks=[logical([0 0]) candidate_checks']';
outliers_new(outlier_checks) = outliers_update;

% this is not a "real" outlier and should be taken into consideration for
% counting the number of samples (only if there are no losts in between)
update = lost<=n_lost;
update(2:end) = update(2:end).*(lost(1:end-1)<=n_lost);
outliers(update) = outliers_new(update);

% situation [x 1 y]: check x with y
candidate_checks = outliers_new(2:end-1)==1;
dist_check = dist2(candidate_checks,:);

outliers_update = 1-fixation_check_normal(dist_check,mu,Sigma,perc_control);

outlier_checks=[logical([0]) candidate_checks']';
outliers_new(outlier_checks) = outliers_update;

%-------------------------------------------------------------------------
function validity=position_check(stamps,max_x,max_y)

% checks whether stamps are in correct position (larger than zero and
% smaller than max_x and max_y
%
% INPUT: 
% stamps   : (n x 2) matrix. (x- and y-coordinates respectively) or (n x 4)
%            x,y left and x,y right
% max_x    : max size x coordinates
% max_y    : max size y coordinates
%
% OUTPUT:
% validity : (n x 1) vector with 1 if valid, zero otherwise

one_eye=size(stamps,2)==2;

if one_eye
    index_valid = logical(stamps(:,1)>0 & stamps(:,2)>0 & stamps(:,1)<max_x+1 & stamps(:,2)<max_y+1);
    validity = zeros(size(stamps,1),1);
    validity(index_valid)=1;
else
    %index_valid = logical(stamps(:,1)>0 & stamps(:,2)>0 & stamps(:,3)>0 & stamps(:,4)>0 & stamps(:,1)<max_x+1 & stamps(:,2)<max_x+1 & stamps(:,3)<max_y+1 & stamps(:,4)<max_y+1);
    % left eye
    index_valid(:,1) = logical(stamps(:,1)>0 & stamps(:,3)>0 & stamps(:,1)<max_x+1 & stamps(:,3)<max_y+1);
    % right eye
    index_valid(:,2) = logical(stamps(:,2)>0 & stamps(:,4)>0 & stamps(:,2)<max_x+1 & stamps(:,4)<max_y+1);
    validity = zeros(size(stamps,1),4); %(both,one_eye,left,right)
    validity(intersect(find(index_valid(:,1)),find(index_valid(:,2))),1)=1;  % both eyes
    validity(union(find(index_valid(:,1)),find(index_valid(:,2))),2)=1;      % one eye
    validity(index_valid(:,1),3)=1;                             % left eye
    validity(index_valid(:,2),4)=1;                             % right eye
end

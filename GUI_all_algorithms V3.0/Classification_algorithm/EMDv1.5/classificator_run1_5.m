...
% Last Modified by GUIDE v2.5 08-Dec-2011 20:13:42
...
% Get state of checkbox
    ...
    elseif( ~get( findobj('Tag', 'Use_IVVT_Model_Checkbox'), 'Value') && ...
            ~get(findobj('Tag', 'Use_IVDT_Model_Checkbox'), 'Value') && ...
            ~get( findobj('Tag', 'Use_IVDT_Model_Checkbox'), 'Value') )
...
% --- Executes on button press in Use_IKF_Model_Checkbox.
function Use_IKF_Model_Checkbox_Callback(hObject, eventdata, handles)
% hObject    handle to Use_IKF_Model_Checkbox (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of Use_IKF_Model_Checkbox

% Create array of all GUI widgets that belongs to I-KF Model
    tmp_widgets = {'IKF_Chi_Threshold';'IKF_Window_Size';'IKF_Deviation'};
% Get state of checkbox
...
% --- Executes on button press in Use_IHMM_Model_Checkbox.
function Use_IHMM_Model_Checkbox_Callback(hObject, eventdata, handles)
% hObject    handle to Use_IHMM_Model_Checkbox (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of Use_IHMM_Model_Checkbox

% Create array of all GUI widgets that belongs to I-HMM Model
    tmp_widgets = {'IHMM_Viterbi_Sample_Size';'IHMM_Baum_Weltch_Reiteration';'IHMM_Saccade_Detection_Threshold'};
% Get state of checkbox
    if(get(hObject,'Value') > 0)
        tmp_enable_state='on';
    else
...
% --- Executes on button press in Use_IMST_Model_Checkbox.
function Use_IMST_Model_Checkbox_Callback(hObject, eventdata, ~)
% hObject    handle to Use_IMST_Model_Checkbox (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of Use_IMST_Model_Checkbox

% Create array of all GUI widgets that belongs to I-HMM Model
    tmp_widgets = {'IMST_Saccade_Detection_Threshold';'IMST_Window_Size'};
% Get state of checkbox
    if(get(hObject,'Value') > 0)
        tmp_enable_state='on';
    else
...
% --- Executes on button press in Use_IDT_Model_Checkbox.
function Use_IDT_Model_Checkbox_Callback(hObject, eventdata, handles)
% hObject    handle to Use_IDT_Model_Checkbox (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of Use_IDT_Model_Checkbox

% Create array of all GUI widgets that belongs to I-HMM Model
    tmp_widgets = {'IDT_Dispersion_Duration_Sec_Threshold';'IDT_Dispersion_Threshold'};
    invoke_state_change = false;
% Get state of checkbox
    if(get(hObject,'Value') > 0), tmp_enable_state='on'; invoke_state_change = true;
    elseif( ~get( findobj('Tag','Use_IVDT_Model_CheckBox'),'Value') ), tmp_enable_state='off'; invoke_state_change = true;
    end
    if( invoke_state_change ),change_model_settings_state(tmp_widgets,tmp_enable_state);end
...
% ========== I-KF Model settings START ==============
    MODEL_SETTINGS.USE.IKF = get(findobj('Tag','Use_IKF_Model_Checkbox'),'Value');
    if(MODEL_SETTINGS.USE.IKF ~= 0)
        model_count=model_count+1;
% Read parameters
        MODEL_SETTINGS.IKF.CHI_THRESHOLD =  str2double( get(findobj('Tag','IKF_Chi_Threshold'),'String') );
        MODEL_SETTINGS.IKF.WINDOW_SIZE =    str2double( get(findobj('Tag','IKF_Window_Size'),'String') );
        MODEL_SETTINGS.IKF.DEVIATION =      str2double( get(findobj('Tag','IKF_Deviation'),'String') );
% Check parameters
        if( (isnan(MODEL_SETTINGS.IKF.CHI_THRESHOLD) || MODEL_SETTINGS.IKF.CHI_THRESHOLD <=0 ) && local_error == 0)
            errordlg('I-KF chi threshold is meaningless. Please provide a correct value','Incorrect chi threshold value.');
            local_error = 4;
        end
        if( (isnan(MODEL_SETTINGS.IKF.WINDOW_SIZE) || MODEL_SETTINGS.IKF.WINDOW_SIZE < 1) && local_error == 0)
            errordlg('I-KF window size is incorrect. Please provide a correct value','Incorrect window size.');
            local_error = 5;
        end
        if( (isnan(MODEL_SETTINGS.IKF.DEVIATION) || MODEL_SETTINGS.IKF.DEVIATION < 1) && local_error == 0)
            errordlg('I-KF deviation value is incorrect. Please enter a correct value','Incorrect deviation.');
            local_error = 6;
        end
    end
% ========== I-KF Model settings END ==============

% ========== I-HMM Model settings START ===========
    MODEL_SETTINGS.USE.IHMM = get(findobj('Tag','Use_IHMM_Model_Checkbox'),'Value');
    if(MODEL_SETTINGS.USE.IHMM ~= 0)
        model_count=model_count+1;
% Read parameters
        MODEL_SETTINGS.IHMM.SACCADE_DETECTION_THRESHOLD = str2double( get(findobj('Tag','IHMM_Saccade_Detection_Threshold'),'String') );
        MODEL_SETTINGS.IHMM.VITERBI_SAMPLE_SIZE = str2double( get(findobj('Tag','IHMM_Viterbi_Sample_Size'),'String') );
        MODEL_SETTINGS.IHMM.BAUM_WELTCH_REITERATIONS = str2double( get(findobj('Tag','IHMM_Baum_Weltch_Reiteration'),'String') );
% Check parameters
        if( (isnan(MODEL_SETTINGS.IHMM.SACCADE_DETECTION_THRESHOLD) || MODEL_SETTINGS.IHMM.SACCADE_DETECTION_THRESHOLD < 1) && local_error == 0)
            errordlg('I-HMM saccade detection threshold value is incorrect. Please enter a correct one','Incorrect threshold value.');
            local_error = 10;
        end
        if( (isnan(MODEL_SETTINGS.IHMM.VITERBI_SAMPLE_SIZE) || MODEL_SETTINGS.IHMM.VITERBI_SAMPLE_SIZE <1 ) && local_error == 0)
            errordlg('I-HMM Viterbi sample size is meaningless. Please provide a correct value','Incorrect Viterbi sample size.');
            local_error = 8;
        end
        if( (isnan(MODEL_SETTINGS.IHMM.BAUM_WELTCH_REITERATIONS) || MODEL_SETTINGS.IHMM.BAUM_WELTCH_REITERATIONS <0) && local_error == 0)
            errordlg('I-HMM Baum-Weltch reiterations count is meaningless. Please provide a correct one','Incorrect reiterations count.');
            local_error = 9;
        end
    end
% ========== I-HMM Model settings END ===========

% ========== I-MST Model settings START =========
    MODEL_SETTINGS.USE.IMST = get(findobj('Tag','Use_IMST_Model_Checkbox'),'Value');
    if(MODEL_SETTINGS.USE.IMST ~= 0)
        model_count=model_count+1;
% Read parameters
        MODEL_SETTINGS.IMST.SACCADE_DETECTION_THRESHOLD = str2double( get(findobj('Tag','IMST_Saccade_Detection_Threshold'),'String') );
        MODEL_SETTINGS.IMST.WINDOW_SIZE = str2double( get(findobj('Tag','IMST_Window_Size'),'String') );
% Check parameters
        if( (isnan(MODEL_SETTINGS.IMST.SACCADE_DETECTION_THRESHOLD) || MODEL_SETTINGS.IMST.SACCADE_DETECTION_THRESHOLD <= 0) && local_error == 0)
            errordlg('I-MST saccade detection threshold value is incorrect. Please enter a correct one','Incorrect threshold value.');
            local_error = 13;
        end
        if( (isnan(MODEL_SETTINGS.IMST.WINDOW_SIZE) || MODEL_SETTINGS.IMST.WINDOW_SIZE < 1) && local_error == 0)
            errordlg('I-MST window size is meaningless. Please provide a correct one','Incorrect window size.');
            local_error = 14;
        end
    end
% ========== I-MST Model settings END ===========

% ========== I-DT Model settings START ==========
% checking for I-DT model settings - but only if corresponding checkbox is
% toggled on
    MODEL_SETTINGS.USE.IDT = get(findobj('Tag','Use_IDT_Model_Checkbox'),'Value');
    if(MODEL_SETTINGS.USE.IDT ~= 0)
        model_count=model_count+1;
% Read parameters
        MODEL_SETTINGS.IDT.DISPERSION_DURATION_SEC_THRESHOLD = str2double( get(findobj('Tag','IDT_Dispersion_Duration_Sec_Threshold'),'String') );
        MODEL_SETTINGS.IDT.DISPERSION_THRESHOLD = str2double( get(findobj('Tag','IDT_Dispersion_Threshold'),'String') );
% Check parameters
        if( (isnan(MODEL_SETTINGS.IDT.DISPERSION_DURATION_SEC_THRESHOLD) || MODEL_SETTINGS.IDT.DISPERSION_DURATION_SEC_THRESHOLD <= 0) && local_error == 0)
            errordlg('I-DT dispersion duration time threshold value is meaningless. Please provide a correct one','Incorrect threshold value.');
            local_error = 17;
        end
        if( (isnan(MODEL_SETTINGS.IDT.DISPERSION_THRESHOLD) || MODEL_SETTINGS.IDT.DISPERSION_THRESHOLD <= 0) && local_error == 0)
            errordlg('I-DT dispersion threshold value is meaningless. Please provide a correct one','Incorrect threshold value.');
            local_error = 18;
        end
    end  
% ========== I-DT Model settings END ============
...
% ========== I-VDT Model settings START ================
    MODEL_SETTINGS.USE.IVDT = get(findobj('Tag','Use_IVDT_Model_Checkbox'),'Value');
    if(MODEL_SETTINGS.USE.IVDT ~= 0)
        model_count=model_count+1;
% Read parameters
        MODEL_SETTINGS.IVDT.SACCADE_DETECTION_THRESHOLD = str2double( get(findobj('Tag','IVT_Saccade_Detection_Threshold'),'String') );
        MODEL_SETTINGS.IVDT.DISPERSION_DURATION_SEC_THRESHOLD = str2double( get(findobj('Tag','IDT_Dispersion_Duration_Sec_Threshold'),'String') );
        MODEL_SETTINGS.IVDT.DISPERSION_THRESHOLD = str2double( get(findobj('Tag','IDT_Dispersion_Threshold'),'String') );
% Check parameters
        if( (isnan(MODEL_SETTINGS.IVDT.DISPERSION_DURATION_SEC_THRESHOLD) || MODEL_SETTINGS.IVDT.DISPERSION_DURATION_SEC_THRESHOLD <= 0) && local_error == 0)
            errordlg('I-VDT dispersion duration time threshold value is meaningless. Please provide a correct one','Incorrect threshold value.');
            local_error = 601;
        end
        if( (isnan(MODEL_SETTINGS.IVDT.DISPERSION_THRESHOLD) || MODEL_SETTINGS.IVDT.DISPERSION_THRESHOLD <= 0) && local_error == 0)
            errordlg('I-VDT dispersion threshold value is meaningless. Please provide a correct one','Incorrect threshold value.');
            local_error = 602;
        end
        if( (isnan(MODEL_SETTINGS.IVDT.SACCADE_DETECTION_THRESHOLD) || MODEL_SETTINGS.IVDT.SACCADE_DETECTION_THRESHOLD < 1) && local_error == 0)
            errordlg('I-VDT Saccade detection threshold value is incorrect. Please enter a correct one','Incorrect threshold value.');
            local_error = 603;
        end
    end
% ========== I-VDT Model settings END ================

% ========== I-VMT Model settings START ================
    MODEL_SETTINGS.USE.IVMT = get(findobj('Tag','Use_IVMT_Model_Checkbox'),'Value');
    if(MODEL_SETTINGS.USE.IVMT ~= 0)
        model_count=model_count+1;
% Read parameters
        MODEL_SETTINGS.IVMT.SACCADE_DETECTION_THRESHOLD = str2double( get(findobj('Tag','IVT_Saccade_Detection_Threshold'),'String') );
        MODEL_SETTINGS.IVMT.TEMPORARY_WINDOW_DURATION = str2double( get(findobj('Tag','IVMT_temporary_window_duration'),'String') );
        MODEL_SETTINGS.IVMT.RANGE_THRESHOLD = str2double( get(findobj('Tag','IVMT_range_threshold'),'String') );
% Check parameters
        if( (isnan(MODEL_SETTINGS.IVMT.TEMPORARY_WINDOW_DURATION) || MODEL_SETTINGS.IVMT.TEMPORARY_WINDOW_DURATION <= 0) && local_error == 0)
            errordlg('I-VMT temporary window duration time is meaningless. Please provide a correct one','Incorrect window length.');
            local_error = 701;
        end
        if( (isnan(MODEL_SETTINGS.IVMT.RANGE_THRESHOLD) || MODEL_SETTINGS.IVMT.RANGE_THRESHOLD <= 0 ) && local_error == 0)
            errordlg('I-VMT range threshold value is meaningless. Please provide a correct one','Incorrect threshold value.');
            local_error = 702;
        end
        if( (isnan(MODEL_SETTINGS.IVMT.SACCADE_DETECTION_THRESHOLD) || MODEL_SETTINGS.IVMT.SACCADE_DETECTION_THRESHOLD < 1) && local_error == 0)
            errordlg('I-VMT Saccade detection threshold value is incorrect. Please enter a correct one','Incorrect threshold value.');
            local_error = 703;
        end
    end
% ========== I-VMT Model settings END ================
...
% selections
    if(local_error == 0)
        ...
        if( MODEL_SETTINGS.USE.IKF ~= 0)
            ...
            used(2) = 1;
            method_str{2} = '_ikf';
            classificator{2}.chi_threshold =    MODEL_SETTINGS.IKF.CHI_THRESHOLD;
            classificator{2}.window_size =      MODEL_SETTINGS.IKF.WINDOW_SIZE;
            classificator{2}.deviation =        MODEL_SETTINGS.IKF.DEVIATION;
        end
        if( MODEL_SETTINGS.USE.IDT ~= 0)
            classificator{3} = classificator_IDT_class;
            used(3) = 1;
            method_str{3} = '_idt';
            classificator{3}.dispersion_duration_sec_threshold =    MODEL_SETTINGS.IDT.DISPERSION_DURATION_SEC_THRESHOLD;
            classificator{3}.dispersion_threshold =                 MODEL_SETTINGS.IDT.DISPERSION_THRESHOLD;
        end
        if( MODEL_SETTINGS.USE.IMST ~= 0)
            classificator{4} = classificator_IMST_class;
            used(4) = 1;
            method_str{4} = '_imst';
            classificator{4}.saccade_detection_threshold =  MODEL_SETTINGS.IMST.SACCADE_DETECTION_THRESHOLD;
            classificator{4}.window_size =                  MODEL_SETTINGS.IMST.WINDOW_SIZE;
        end
        if( MODEL_SETTINGS.USE.IHMM ~= 0)
            classificator{5} = classificator_IHMM_class;
            used(5) = 1;
            method_str{5} = '_ihmm';
            classificator{5}.saccade_detection_threshold =      MODEL_SETTINGS.IHMM.SACCADE_DETECTION_THRESHOLD;
            classificator{5}.viterbi_sample_size =              MODEL_SETTINGS.IHMM.VITERBI_SAMPLE_SIZE;
            classificator{5}.baum_welch_reiterations_count =    MODEL_SETTINGS.IHMM.BAUM_WELTCH_REITERATIONS;
        end
        if( MODEL_SETTINGS.USE.IVVT ~= 0)
            classificator{6} = classificator_IVVT_class;
            used(6) = 1;
            method_str{6} = '_ivvt';
            classificator{6}.saccade_detection_threshold = MODEL_SETTINGS.IVVT.SACCADE_DETECTION_THRESHOLD;
            classificator{6}.fixation_detection_threshold = MODEL_SETTINGS.IVVT.FIXATION_DETECTION_THRESHOLD;
        end
        if( MODEL_SETTINGS.USE.IVDT ~= 0)
            classificator{7} = classificator_IVDT_class;
            used(7) = 1;
            method_str{7} = '_ivdt';
            classificator{7}.saccade_detection_threshold = MODEL_SETTINGS.IVDT.SACCADE_DETECTION_THRESHOLD;
            classificator{7}.dispersion_duration_sec_threshold = MODEL_SETTINGS.IVDT.DISPERSION_DURATION_SEC_THRESHOLD;
            classificator{7}.idt_dispersion_threshold = MODEL_SETTINGS.IVDT.DISPERSION_THRESHOLD;
        end
        if( MODEL_SETTINGS.USE.IVMT ~= 0)
            classificator{8} = classificator_IVMT_class;
            used(8) = 1;
            method_str{8} = '_ivmt';
            classificator{8}.saccade_detection_threshold = MODEL_SETTINGS.IVMT.SACCADE_DETECTION_THRESHOLD;
            classificator{8}.window_duration_sec_threshold = MODEL_SETTINGS.IVMT.TEMPORARY_WINDOW_DURATION;
            classificator{8}.san_agustin_threshold = MODEL_SETTINGS.IVMT.RANGE_THRESHOLD;
        end
                ...
                if( classificator{i}.error_code == 0 )
                    ...
                    if( MODEL_SETTINGS.PROCESSING.PLOTS.USE ~= 0 || MODEL_SETTINGS.PROCESSING.SCORES.USE ~= 0)
                        ...
                        if( MODEL_SETTINGS.PROCESSING.SCORES.USE ~= 0)
                            if( MODEL_SETTINGS.READER.SAMPLE_RATE ~=1000 )
                                scores_computator.generate_stimulus_CS_TR_2009_16();
                            else
                                scores_computator.selected_stimulus = 4;
                                scores_computator.generate_current_stimulus_1000Hz();
                                scores_computator.generate_stimulus_1000();
                            end
                        end
                        ...
                        if( MODEL_SETTINGS.PROCESSING.SCORES.USE ~= 0)
                            data{ i,1 } = scores_computator.FQnS;
                            data{ i,2 } = scores_computator.FQlS;
                            data{ i,3 } = scores_computator.SQnS;
                            data{ i,4 } = scores_computator.AFD;
                            data{ i,5 } = scores_computator.ANF;
                            data{ i,6 } = scores_computator.ASA;
                            data{ i,7 } = scores_computator.ANS;
                            data{ i,8 } = scores_computator.PQnS;
                            data{ i,9 } = scores_computator.FPMS;
                            data{ i,10} = scores_computator.SPMS;
                            data{ i,11} = scores_computator.PQlS_P;
                            data{ i,12} = scores_computator.PQlS_V;
                        end
...
% --- Executes during object creation, after setting all properties.
function IKF_Chi_Threshold_CreateFcn(hObject, eventdata, handles)
% hObject    handle to IKF_Chi_Threshold (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes during object creation, after setting all properties.
function IKF_Window_Size_CreateFcn(hObject, eventdata, handles)
% hObject    handle to IKF_Window_Size (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes during object creation, after setting all properties.
function IKF_Deviation_CreateFcn(hObject, eventdata, handles)
% hObject    handle to IKF_Deviation (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes during object creation, after setting all properties.
function IHMM_Viterbi_Sample_Size_CreateFcn(hObject, eventdata, handles)
% hObject    handle to IHMM_Viterbi_Sample_Size (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes during object creation, after setting all properties.
function IHMM_Baum_Weltch_Reiteration_CreateFcn(hObject, eventdata, handles)
% hObject    handle to IHMM_Baum_Weltch_Reiteration (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes during object creation, after setting all properties.
function IHMM_Saccade_Detection_Threshold_CreateFcn(hObject, eventdata, handles)
% hObject    handle to IHMM_Saccade_Detection_Threshold (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes during object creation, after setting all properties.
function IHMM_Fixation_Detection_Threshold_CreateFcn(hObject, eventdata, handles)
% hObject    handle to IHMM_Fixation_Detection_Threshold (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes during object creation, after setting all properties.
function IMST_Saccade_Detection_Threshold_CreateFcn(hObject, eventdata, handles)
% hObject    handle to IMST_Saccade_Detection_Threshold (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes during object creation, after setting all properties.
function IMST_Fixation_Detection_Threshold_CreateFcn(hObject, eventdata, handles)
% hObject    handle to IMST_Fixation_Detection_Threshold (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes during object creation, after setting all properties.
function IMST_Window_Size_CreateFcn(hObject, eventdata, handles)
% hObject    handle to IMST_Window_Size (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes during object creation, after setting all properties.
function IDT_Dispersion_Duration_Sec_Threshold_CreateFcn(hObject, eventdata, handles)
% hObject    handle to IDT_Dispersion_Duration_Sec_Threshold (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes during object creation, after setting all properties.
function IDT_Dispersion_Threshold_CreateFcn(hObject, eventdata, handles)
% hObject    handle to IDT_Dispersion_Threshold (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
function IKF_Chi_Threshold_Callback(hObject, eventdata, handles)
% hObject    handle to IKF_Chi_Threshold (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of IKF_Chi_Threshold as text
%        str2double(get(hObject,'String')) returns contents of IKF_Chi_Threshold as a double



function IKF_Window_Size_Callback(hObject, eventdata, handles)
% hObject    handle to IKF_Window_Size (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of IKF_Window_Size as text
%        str2double(get(hObject,'String')) returns contents of IKF_Window_Size as a double



function IKF_Deviation_Callback(hObject, eventdata, handles)
% hObject    handle to IKF_Deviation (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of IKF_Deviation as text
%        str2double(get(hObject,'String')) returns contents of IKF_Deviation as a double



function IMST_Saccade_Detection_Threshold_Callback(hObject, eventdata, handles)
% hObject    handle to IMST_Saccade_Detection_Threshold (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of IMST_Saccade_Detection_Threshold as text
%        str2double(get(hObject,'String')) returns contents of IMST_Saccade_Detection_Threshold as a double



function IMST_Window_Size_Callback(hObject, eventdata, handles)
% hObject    handle to IMST_Window_Size (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of IMST_Window_Size as text
%        str2double(get(hObject,'String')) returns contents of IMST_Window_Size as a double



function IDT_Dispersion_Duration_Sec_Threshold_Callback(hObject, eventdata, handles)
% hObject    handle to IDT_Dispersion_Duration_Sec_Threshold (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of IDT_Dispersion_Duration_Sec_Threshold as text
%        str2double(get(hObject,'String')) returns contents of IDT_Dispersion_Duration_Sec_Threshold as a double



function IDT_Dispersion_Threshold_Callback(hObject, eventdata, handles)
% hObject    handle to IDT_Dispersion_Threshold (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of IDT_Dispersion_Threshold as text
%        str2double(get(hObject,'String')) returns contents of IDT_Dispersion_Threshold as a double



function IHMM_Saccade_Detection_Threshold_Callback(hObject, eventdata, handles)
% hObject    handle to IHMM_Saccade_Detection_Threshold (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of IHMM_Saccade_Detection_Threshold as text
%        str2double(get(hObject,'String')) returns contents of IHMM_Saccade_Detection_Threshold as a double



function IHMM_Viterbi_Sample_Size_Callback(hObject, eventdata, handles)
% hObject    handle to IHMM_Viterbi_Sample_Size (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of IHMM_Viterbi_Sample_Size as text
%        str2double(get(hObject,'String')) returns contents of IHMM_Viterbi_Sample_Size as a double



function IHMM_Baum_Weltch_Reiteration_Callback(hObject, eventdata, handles)
% hObject    handle to IHMM_Baum_Weltch_Reiteration (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of IHMM_Baum_Weltch_Reiteration as text
%        str2double(get(hObject,'String')) returns contents of IHMM_Baum_Weltch_Reiteration as a double
...
% --- Executes on button press in Use_IVDT_Model_Checkbox.
function Use_IVDT_Model_Checkbox_Callback(hObject, eventdata, handles)
% hObject    handle to Use_IVDT_Model_Checkbox (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of Use_IVDT_Model_Checkbox
    tmp_widgets = {'IDT_Dispersion_Duration_Sec_Threshold';'IDT_Dispersion_Threshold'};
    invoke_state_change = false;
% Get state of checkbox
    if(get(hObject,'Value') > 0), tmp_enable_state='on'; invoke_state_change = true;
    elseif( ~get( findobj('Tag', 'Use_IDT_Model_Checkbox'), 'Value') ), tmp_enable_state='off'; invoke_state_change = true;
    end
    if( invoke_state_change ), change_model_settings_state(tmp_widgets,tmp_enable_state); end
...
% Get state of checkbox
    if(get(hObject,'Value') > 0),tmp_enable_state='on'; invoke_state_change = true;
    elseif( ~get( findobj('Tag', 'Use_IVVT_Model_Checkbox'), 'Value') && ...
            ~get( findobj('Tag', 'Use_IVT_Model_Checkbox'), 'Value') && ...
            ~get( findobj('Tag', 'Use_IVDT_Model_Checkbox'), 'Value') )
        tmp_enable_state='off';
        invoke_state_change = true;
    end
...
function IVMT_temporary_window_duration_Callback(hObject, eventdata, handles)
% hObject    handle to IVMT_temporary_window_duration (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of IVMT_temporary_window_duration as text
%        str2double(get(hObject,'String')) returns contents of IVMT_temporary_window_duration as a double


% --- Executes during object creation, after setting all properties.
function IVMT_temporary_window_duration_CreateFcn(hObject, eventdata, handles)
% hObject    handle to IVMT_temporary_window_duration (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
function IVMT_range_threshold_Callback(hObject, eventdata, handles)
% hObject    handle to IVMT_range_threshold (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of IVMT_range_threshold as text
%        str2double(get(hObject,'String')) returns contents of IVMT_range_threshold as a double


% --- Executes during object creation, after setting all properties.
function IVMT_range_threshold_CreateFcn(hObject, eventdata, handles)
% hObject    handle to IVMT_range_threshold (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on button press in Use_IVMT_Model_Checkbox.
function Use_IVMT_Model_Checkbox_Callback(hObject, eventdata, handles)
% hObject    handle to Use_IVMT_Model_Checkbox (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of Use_IVMT_Model_Checkbox
% Hint: get(hObject,'Value') returns toggle state of Use_IVDT_Model_Checkbox
    tmp_widgets = {'IVMT_temporary_window_duration';'IVMT_range_treshold'};
% Get state of checkbox
    if(get(hObject,'Value') > 0), tmp_enable_state='on'; else tmp_enable_state='off'; end
    change_model_settings_state(tmp_widgets,tmp_enable_state);

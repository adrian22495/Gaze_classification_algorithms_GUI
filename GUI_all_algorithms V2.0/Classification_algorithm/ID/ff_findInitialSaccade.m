function [saccadeSampleIds, peakVelocity] = ff_findInitialSaccade(stim)
...
sampleTime = mean(gradient(stim.T)); % Average time between samples (should be close to 0.016)
...
% Convert mm into degrees assuming a view distance of 50cm
velocities = mm2deg(velocities, 500);
distances = mm2deg(distances, 500);

function stim = compile_stim_info(test)
...
for i=1:size(stim,1)
    if stim{i}.valid
        [stim{i}.initialSaccade, stim{i}.saccadicPeakV] = ff_findInitialSaccade(stim{i});
    end
end
...
for i=1:size(stim,1)
    if stim{i}.valid
        [SRT] = ff_compute_SRT_oneStim(stim{i}.initialSaccade);
        stim{i}.SRT = SRT;
...
if ~test.params.f_appv1
    ...
    % Can't compute SP for this test
    for i=1:size(stim,1)
        stim{i}.SP = NaN;
    end
end

...
for k = 1:max(velLabeled)
    ...
    if length(peakIdx) < minPeakSamples
        ETparams.nanIndx(i,j).Idx(peakIdx) = 1;
        continue
    end
    ...
    if localVelNoise > ETparams.data(i,j).peakDetectionThreshold
        ETparams.nanIndx(i,j).Idx(saccadeStartIdx:peakIdx) = 1;
        continue
    end
    ...
    if any(ETparams.nanIdx(i,j).Idx(saccadeStartIdx:saccadeEndIdx))
        ETparams.nanIndx(i,j).Idx(saccadeStartIdx:saccadeEndIdx) = 1;
        continue
    end
